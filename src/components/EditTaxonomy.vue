<template>
  <b-row>
    <b-col cols="12">
      <h2>
        Edit Taxonomy
        <router-link :to="{ name: 'ShowTaxonomy', params: { id: book._id } }">(Show Taxonomy)</router-link>
      </h2>
...
export default {
  ...
  methods: {
    onSubmit (evt) {
      evt.preventDefault()
      axios.put(`http://192.168.99.100:3000/taxonomy/` + this.$route.params.id, this.book)
      .then(response => {
        this.$router.push({
          name: 'ShowTaxonomy',
